 ...
 * Used to validate customer existence and retrieve customer type/profile.
 */
@Slf4j
@Component
public class CustomerClient {
     ...
     * Throws BusinessValidationException if customer not found.
     *
     * @param customerId the customer ID to look up
     * @return a Map with the customer's data fields
     */
    ...
    public Map<String, Object> findCustomerById(String customerId) {
        try {
            ...
            throw new BusinessValidationException("Customer not found with id: " + customerId);
        } catch (Exception ex) {
            log.error("Error calling ms-customer: {}", ex.getMessage());
            throw new BusinessValidationException("Error communicating with customer service");
        }
    }
}